...
param(
    ...
    [string]$ResultsRoot = "C:\zhongliu\zhongliu-tuning\protocol-40-10-38\mask-appearance-validation"
)

$ErrorActionPreference = "Stop"
$Runner = Join-Path $RepoRoot "scripts\run_hierarchical_followup_resumable.ps1"
$Summarizer = Join-Path $RepoRoot "scripts\summarize_mask_appearance_validation.ps1"
...
try {
    ...
    Write-Host "[$([DateTimeOffset]::Now.ToString('o'))] Mask appearance queue started"
    Write-Host "The 40-case split is audited but not read by this parameter-free study."
    foreach ($Split in @(
        ...
        & $Summarizer `
            -Results $SplitResults `
            -Prefix "mask-appearance-$($Split.Name)"
    }
    Write-Host "[$([DateTimeOffset]::Now.ToString('o'))] Mask appearance queue completed"
}
catch {
