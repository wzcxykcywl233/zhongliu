...
$ReferenceName = "hierarchical_full_grid0_iterations2_memory_topk"
...
$Deltas = foreach ($Name in $Candidates) {
    ...
    if (-not $Row) { throw "Missing mask appearance result: $Name" }
    [PSCustomObject]@{
        Profile = $Name
...
$DeltaPath = Join-Path $Results "$Prefix-deltas-vs-memory-topk.csv"
...
Write-Host "Candidate minus memory_topk: positive DSC/D98 and negative HD95/MASD/CD indicate improvement."
